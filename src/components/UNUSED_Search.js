...
{    
    ...
    const {searchFields, tableName, constants, fields, handleStatus, setList, handleUpArrow} = props
    ...
    const searchRecords = searchValues =>
    {
        ...
        serverFetch(link, '', '', list=>{
            if (list.length > 0) {
                if (handleStatus) {handleStatus({color:'white', backgroundColor:'green'}, '')}
                setList(list.sort((a,b) => b.id - a.id))
            } else {
                if (handleStatus) {handleStatus({color:'white', backgroundColor:'green'}, 'Not found')}
            }   
        })
    }        
